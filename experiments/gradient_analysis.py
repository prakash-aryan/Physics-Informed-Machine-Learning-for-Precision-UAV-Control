# experiments/gradient_analysis.py
import sys, os
...
def run_gradient_analysis(config_name='full_piatsg', num_steps=1000, log_interval=10):
    ...
    for step in range(num_steps):
        ...
        if len(agent.memory) >= config.batch_size and step % log_interval == 0:
            # Sample batch - the buffer returns tensors directly
            batch = agent.memory.sample(config.batch_size)
            
            # Batch is a tuple of (states, actions, rewards, next_states, dones)
...
def plot_gradient_analysis(logger, save_path='gradient_analysis.png'):
    ...
    ax1.set_title('Gradient Magnitudes by Component')
    ax1.legend()
    ...
    ax2.set_title('Gradient Alignment Scores')
    ax2.legend()
...
if __name__ == '__main__':
    ...
    logger = run_gradient_analysis(
        config_name='full_piatsg',
        num_steps=1000,
        log_interval=10
    )
    
    # Plot results
    plot_gradient_analysis(logger, save_path='gradient_logs/full_piatsg/analysis.png')